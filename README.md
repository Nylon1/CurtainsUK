...
## Build My Rooms release check, 22 September 2026

Owner authorised the scoped live release after fixture commit `1ff71b1`. The final read-only comparison confirmed theme `182339731835` is live, all five shared deployment files still match the reconciled baseline, and all ten checked Customer Trust surfaces are preserved. No live theme mutation has been made. The source now includes the public House checkout operation behind its own server-side release gate. It promotes only an already revalidated immutable House contract to the established production Draft Order transport, returns a host-validated Shopify checkout URL only after exact reconciliation and records the House execution durably. On 22 September, the genuine signed Shopify app-proxy route completed a private two-room, three-curtain proof: £1,828.00 goods, £306.83 VAT, one £12.95 delivery charge and £1,840.95 total matched Shopify exactly. The URL was not opened, no payment or customer email occurred, and the private Draft Order was deleted. The temporary House feature/release gates were then removed again. The feature remains unconnected to the Shopify theme pending Astra integration. See [the Build My Rooms runbook](docs/BUILD_MY_ROOMS_V1.md#public-house-checkout-operation-22-september-2026).

## Build My Rooms V1 — unpublished feature, 21 September 2026

Development lives on `feature/build-my-rooms-v1`, in the isolated `curtainsuk-build-my-rooms` worktree. See [Build My Rooms implementation and release runbook](docs/BUILD_MY_ROOMS_V1.md). The room experience, structured device autosave, immutable curtain retention and server revalidation adapter are implemented for local review. The source now contains a relational multi-curtain House extension (one paid order → many immutable curtain snapshots) with HMAC-verified webhook reconstruction, whole-House staff-only release and durable House idempotency. On 22 September 2026, migration `20260921205826_multi_curtain_paid_order_contract` was applied and a private two-room/three-curtain Draft Order rehearsal reconciled exactly: £2,149.00 goods, £360.33 VAT, one £12.95 delivery line and £2,161.95 total. The invisible Draft Order was deleted, no invoice/payment/email/workroom release occurred, and public House checkout remains disabled. No production pricing, stock, paid lifecycle meaning, workroom policy, Customer Trust or cookie implementation was changed.
...
Final integration on 22 September connects Continue to signed House review verification, fresh whole-House validation, per-curtain immutable snapshot preparation and the existing multi-line contract. Public execution remains explicitly disabled; the separate nonpayable server binding uses existing durable claims/recovery/persistence. Exact retained identity, room choice, price acknowledgements, combined stock and browser persistence have local coverage. This new bridge has **not** been deployed or rehearsed against live Shopify. See the [final integration evidence and limits](artifacts/build-my-rooms-v1/final-integration.md); owner visual approval and scoped canonical-live reconciliation are still required before publication.
