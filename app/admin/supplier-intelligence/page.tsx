...
export default function SupplierIntelligencePage() {
  ...
  return <main className="min-h-screen bg-[#102c26] text-white"><div className="mx-auto max-w-7xl px-5 py-10">
    <div className="flex flex-wrap items-start justify-between gap-4"><div><p className="text-xs uppercase tracking-[0.2em] text-[#f1cf8a]">Private supplier operations</p><h1 className="mt-3 text-4xl font-semibold">Supplier Sync Health</h1><p className="mt-3 max-w-3xl text-white/60">Shadow snapshots require validation and explicit staff approval. This screen does not write availability to Shopify.</p></div><div className="flex gap-3"><Link href="/admin" className="rounded-full border border-white/15 px-5 py-3 text-sm">Admin home</Link><Link href="/admin/prestigious-stock" className="rounded-full bg-[#f1cf8a] px-5 py-3 text-sm font-semibold text-[#102c26]">Stock verification</Link></div></div>
    ...
    <div className="mt-6 grid gap-6 lg:grid-cols-2">
      ...
      <section className="rounded-3xl border border-white/10 bg-white/[0.04] p-6"><h2 className="text-xl font-semibold">Drill-down</h2><label className="mt-4 block text-sm text-white/60">Supplier<select value={supplierId} disabled className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3"><option value={supplierId}>Prestigious Textiles</option></select></label><label className="mt-4 block text-sm text-white/60">Filter SKU<input value={skuFilter} onChange={(event) => setSkuFilter(event.target.value)} placeholder="e.g. 4269/147" className="mt-2 w-full rounded-xl border border-white/10 bg-black/20 px-4 py-3" /></label></section>
    </div>
