...
import { supplierPriceApprovalIdentity } from "@/lib/supplier-intelligence/server-auth";
import { isPriceOnlyObservation } from "@/lib/supplier-intelligence/authz";
...
export async function POST(request: NextRequest) {
  const admin = await supplierPriceApprovalIdentity();
  ...
  try {
    if (admin.priceOnly && (body.decision !== "APPROVE" || !isPriceOnlyObservation(await repository.snapshot(body.snapshotId)))) {
      return NextResponse.json({ error: "SUPPLIER_PRICE_ONLY_APPROVAL_REQUIRED" }, { status: 403 });
    }
