...
export default async function Page({ params, searchParams }: Props) {
  ...
  const invitationUrl = query.invite
    ? `https://www.apexcurtains.com/professionals/workspace/invitations/${query.invite}`
    : null;
      ...
      <div className="mx-auto max-w-6xl">
        ...
        <div className="mt-6 flex flex-col gap-4 lg:flex-row lg:items-end lg:justify-between">
          <div>
            ...
            <p className="mt-4 max-w-3xl text-[#C8D1D8]">Access is project-scoped. Invitations carry an explicit professional role and organisation so the workspace records who is contributing to the project and in what capacity.</p>
          </div>
        ...
        {invitationUrl && (
          <section className="mt-8 rounded-[28px] border border-[#d6b56b]/25 bg-[#d6b56b]/10 p-6">
            <p className="text-xs font-semibold uppercase tracking-[0.18em] text-[#d6b56b]">Invitation created</p>
            <h2 className="mt-3 text-xl font-semibold">Copy this single-use project invitation link</h2>
            <code className="mt-4 block overflow-x-auto rounded-2xl border border-white/10 bg-black/20 p-4 text-sm text-[#F4F0E8]">{invitationUrl}</code>
            <p className="mt-3 text-sm text-[#C8D1D8]">The recipient must sign in with the same email address used for the invitation. The link expires after 14 days.</p>
          </section>
        )}
            ...
            {data.isOwner && (
              <form action={createProjectInvitation.bind(null, id)} className="rounded-[30px] border border-white/10 bg-[#1B405B] p-7">
                ...
                <div className="mt-5 space-y-4">
                  ...
                  <button className="w-full rounded-full bg-[#d6b56b] px-5 py-3 text-sm font-semibold text-apex-navy-950">Create invitation link</button>
                </div>
              </form>
            )}
