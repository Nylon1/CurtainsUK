...
export default function CurtainConfigurator({ fabrics, initialWindow, initialFabric }: { fabrics: [CustomerSafeFabricProjection, ...CustomerSafeFabricProjection[]]; initialWindow?: string; initialFabric?: string }) {
  ...
  const [form, setForm] = useState<FormState>({
    windowSlug: safeWindow, measurementBasis: "TRACK_WIDTH", widthCm: "", dropCm: "", baySegments: "", bayTrackOrPoleFitted: null, cornerSegments: "", cornerAngle: "", peakHeight: "", leftVertical: "", rightVertical: "", leftSlope: "", rightSlope: "", leftAngle: "", rightAngle: "", fixingPosition: "Wall fixed above glazing", heading: "PENCIL_PLEAT", fabricId: safeFabric, liningChoice: "STANDARD", construction: "PAIR", stackDirection: "SPLIT", extras: [], photoNames: [], drawingName: "",
  });
  ...
  const awkward = form.windowSlug === "awkward-unusual-window";
  const curved = form.windowSlug === "curved-bow-window";
  const corner = form.windowSlug === "corner-window";
  ...
  function validateCurrentStep() {
    ...
    if (step === 1) {
      if (specialist) {
        const required = awkward
          ? [form.widthCm, form.dropCm]
          : [form.widthCm, form.peakHeight, form.leftVertical, form.rightVertical, form.leftSlope, form.rightSlope];
        if (required.some((value) => !Number.isFinite(Number(value)) || Number(value) <= 0)) return awkward ? "Enter a rough overall width and maximum drop." : "Enter every required specialist measurement in centimetres.";
        ...
        if (awkward && !form.drawingName) return "Add a simple drawing of the unusual window.";
      } else if (form.windowSlug === "bay-window") {
        const segments = numericList(form.baySegments);
        if (form.bayTrackOrPoleFitted === null) return "Tell us whether a track or pole is already fitted.";
        if (segments.length < 2 || segments.length > 8) return "Enter between two and eight bay section widths, separated by commas.";
        if (!Number.isFinite(Number(form.dropCm)) || Number(form.dropCm) <= 0) return "Enter the finished drop.";
      } else if (corner) {
        const segments = numericList(form.cornerSegments);
        const angle = Number(form.cornerAngle);
        if (segments.length !== 2 || segments.some((value) => value < 10 || value > 600)) return "Enter exactly two corner section widths between 10 cm and 600 cm.";
        if (!Number.isFinite(angle) || angle < 1 || angle > 359) return "Enter one corner angle between 1 and 359 degrees.";
        if (!Number.isFinite(Number(form.dropCm)) || Number(form.dropCm) <= 0) return "Enter the finished drop.";
        if (!form.photoNames.length) return "Add at least one project photograph.";
      } else if (![form.widthCm, form.dropCm].every((value) => Number.isFinite(Number(value)) && Number(value) > 0)) return curved ? "Enter a valid track arc length and finished drop." : "Enter a valid coverage width and finished drop.";
      else if (curved && !form.photoNames.length) return "Add at least one project photograph.";
    }
    return "";
  }
    ...
    try {
      if (specialist) {
        const response = await fetch("/api/staging/specialist-review", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({
          ...
          measurements: awkward
            ? { coverage_width: Number(form.widthCm), finished_drop: Number(form.dropCm) }
          ...
          fabricId: form.fabricId, heading: form.heading, lining: form.liningChoice === "INTERLINING" ? "STANDARD" : form.liningChoice, construction: form.construction, fixingPosition: form.fixingPosition, stackDirection: form.stackDirection, photoNames: form.photoNames, drawingName: form.drawingName || undefined,
        }) });
        ...
        const segments = numericList(form.baySegments);
        const cornerSections = numericList(form.cornerSegments);
        const width = form.windowSlug === "bay-window" || corner ? undefined : Number(form.widthCm);
        const response = await fetch("/api/staging/curtains-price", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ windowSlug: form.windowSlug, measurementBasis: form.windowSlug === "bay-window" || corner || curved ? "TRACK_WIDTH" : form.measurementBasis, widthCm: width, dropCm: Number(form.dropCm), bayTrackOrPoleFitted: form.bayTrackOrPoleFitted, bayNumberOfSections: form.windowSlug === "bay-window" ? segments.length : undefined, baySegmentWidthsCm: form.windowSlug === "bay-window" ? segments : undefined, cornerSectionWidthsCm: corner ? cornerSections : undefined, cornerAngleDegrees: corner ? Number(form.cornerAngle) : undefined, fabricId: form.fabricId, heading: form.heading, lining: form.liningChoice === "INTERLINING" ? "STANDARD" : form.liningChoice, interlining: form.liningChoice === "INTERLINING" ? "INTERLINING" : "NONE", construction: form.construction, stackDirection: form.stackDirection, photoNames: form.photoNames }) });
    ...
    <div className="mx-auto max-w-[1320px]">
      ...
      <div className="grid gap-7 lg:grid-cols-[1fr_340px]">
        <section className="min-h-[560px] rounded-[30px] border border-[#173c32]/10 bg-white p-5 shadow-[0_18px_50px_rgba(23,60,50,0.08)] sm:p-8">
          ...
          {step === 1 && !specialist && <div><div className="text-xs font-semibold uppercase tracking-[0.16em] text-[#996a31]">Measurements · centimetres</div><h1 className="mt-3 text-3xl font-semibold tracking-[-0.04em]">Measure the full curtain coverage</h1>{form.windowSlug === "bay-window" ? <div className="mt-7 grid gap-5"><div><div className="text-sm font-semibold">Is a track or pole already fitted?</div><div className="mt-3 grid gap-3 sm:grid-cols-2"><ChoiceButton selected={form.bayTrackOrPoleFitted === true} title="Yes" detail="Measure each fitted section" onClick={() => update("bayTrackOrPoleFitted", true)} /><ChoiceButton selected={form.bayTrackOrPoleFitted === false} title="No" detail="Enter the intended curtain coverage by section" onClick={() => update("bayTrackOrPoleFitted", false)} /></div></div><Field label="Bay section widths" value={form.baySegments} onChange={(value) => update("baySegments", value)} suffix="cm, comma-separated" placeholder="80, 180, 80" /><Field label="Finished curtain drop" value={form.dropCm} onChange={(value) => update("dropCm", value)} /><label className="rounded-2xl border border-dashed border-[#173c32]/25 bg-[#f7f3ec] p-5 text-sm font-semibold"><span className="flex items-center gap-2"><Upload className="h-4 w-4" /> Photograph of the full bay (optional)</span><input type="file" accept="image/*" multiple onChange={(event) => update("photoNames", Array.from(event.target.files ?? []).map((file) => file.name))} className="mt-3 block w-full text-xs font-normal" /><button type="button" onClick={() => update("photoNames", ["demo-bay-window.jpg"])} className="mt-3 rounded-full border border-[#173c32]/15 bg-white px-3 py-2 text-xs font-semibold">Use demo photograph</button><span className="mt-2 block text-xs font-normal text-[#687a73]">Angles are not required. The total coverage is calculated from the section widths.</span></label></div> : corner ? <div className="mt-7 grid gap-5 sm:grid-cols-2"><Field label="Two corner section widths" value={form.cornerSegments} onChange={(value) => update("cornerSegments", value)} suffix="cm, comma-separated" placeholder="160, 140" /><Field label="Corner angle" value={form.cornerAngle} onChange={(value) => update("cornerAngle", value)} suffix="degrees" placeholder="e.g. 90" /><Field label="Finished curtain drop" value={form.dropCm} onChange={(value) => update("dropCm", value)} /><label className="rounded-2xl border border-dashed border-[#173c32]/25 bg-[#f7f3ec] p-5 text-sm font-semibold"><span className="flex items-center gap-2"><Upload className="h-4 w-4" /> Corner photograph (required)</span><input type="file" accept="image/*" multiple onChange={(event) => update("photoNames", Array.from(event.target.files ?? []).map((file) => file.name))} className="mt-3 block w-full text-xs font-normal" /></label></div> : curved ? <div className="mt-7 grid gap-5 sm:grid-cols-2"><Field label="Track arc length" value={form.widthCm} onChange={(value) => update("widthCm", value)} placeholder="e.g. 260" /><Field label="Finished curtain drop" value={form.dropCm} onChange={(value) => update("dropCm", value)} placeholder="e.g. 220" /><div className="sm:col-span-2 rounded-2xl bg-[#f7f3ec] p-4 text-xs leading-5 text-[#60746d]"><Ruler className="mr-2 inline h-4 w-4 text-[#a36d2d]" />Measure along the complete curved track, not straight across the opening.</div><label className="sm:col-span-2 rounded-2xl border border-dashed border-[#173c32]/25 bg-[#f7f3ec] p-5 text-sm font-semibold"><span className="flex items-center gap-2"><Upload className="h-4 w-4" /> Curved window photograph (required)</span><input type="file" accept="image/*" multiple onChange={(event) => update("photoNames", Array.from(event.target.files ?? []).map((file) => file.name))} className="mt-3 block w-full text-xs font-normal" /></label></div> : <div className="mt-7 grid gap-5 sm:grid-cols-2"><div className="sm:col-span-2"><div className="text-sm font-semibold">What did you measure?</div><div className="mt-3 grid gap-3 sm:grid-cols-2"><ChoiceButton selected={form.measurementBasis === "TRACK_WIDTH"} title="Track width" detail="The complete usable track" onClick={() => update("measurementBasis", "TRACK_WIDTH")} /><ChoiceButton selected={form.measurementBasis === "POLE_USABLE_WIDTH"} title="Pole usable width" detail="Between finials or end stops" onClick={() => update("measurementBasis", "POLE_USABLE_WIDTH")} /></div></div><Field label="Full coverage width" value={form.widthCm} onChange={(value) => update("widthCm", value)} placeholder="e.g. 240" /><Field label="Finished drop" value={form.dropCm} onChange={(value) => update("dropCm", value)} placeholder="e.g. 220" /><div className="sm:col-span-2 rounded-2xl bg-[#f7f3ec] p-4 text-xs leading-5 text-[#60746d]"><Ruler className="mr-2 inline h-4 w-4 text-[#a36d2d]" />Enter the space the curtains cover, not the finished width of an existing curtain.</div></div>}</div>}

          {step === 1 && specialist && <div><div className="text-xs font-semibold uppercase tracking-[0.16em] text-[#996a31]">{awkward ? "Rough project dimensions" : "Specialist geometry"} · centimetres</div><h1 className="mt-3 text-3xl font-semibold tracking-[-0.04em]">{awkward ? "Help us understand the unusual window" : `Map every edge of the ${windowType.name.toLowerCase()}`}</h1><p className="mt-3 text-sm leading-6 text-[#667a72]">These measurements create a review record only. They cannot release a curtain into payment or manufacture.</p><div className="mt-7 grid gap-5 sm:grid-cols-2">{awkward ? <><Field label="Rough overall width" value={form.widthCm} onChange={(value) => update("widthCm", value)} /><Field label="Rough maximum drop / height" value={form.dropCm} onChange={(value) => update("dropCm", value)} /></> : <><Field label="Base width" value={form.widthCm} onChange={(value) => update("widthCm", value)} /><Field label="Overall peak height" value={form.peakHeight} onChange={(value) => update("peakHeight", value)} /><Field label="Left vertical" value={form.leftVertical} onChange={(value) => update("leftVertical", value)} /><Field label="Right vertical" value={form.rightVertical} onChange={(value) => update("rightVertical", value)} /><Field label="Left slope" value={form.leftSlope} onChange={(value) => update("leftSlope", value)} /><Field label="Right slope" value={form.rightSlope} onChange={(value) => update("rightSlope", value)} /><Field label="Left slope angle (optional)" value={form.leftAngle} onChange={(value) => update("leftAngle", value)} suffix="degrees" /><Field label="Right slope angle (optional)" value={form.rightAngle} onChange={(value) => update("rightAngle", value)} suffix="degrees" /></>}<label className="sm:col-span-2 block text-sm font-semibold">Proposed fixing position<input value={form.fixingPosition} onChange={(event) => update("fixingPosition", event.target.value)} className="mt-2 w-full rounded-xl border border-[#173c32]/15 px-4 py-3 font-normal outline-none" /></label><label className="rounded-2xl border border-dashed border-[#173c32]/25 bg-[#f7f3ec] p-5 text-sm font-semibold"><span className="flex items-center gap-2"><Upload className="h-4 w-4" /> Project photographs (required)</span><input type="file" accept="image/*" multiple onChange={(event) => update("photoNames", Array.from(event.target.files ?? []).map((file) => file.name))} className="mt-3 block w-full text-xs font-normal" /><button type="button" onClick={() => update("photoNames", ["demo-specialist-window.jpg"])} className="mt-3 rounded-full border border-[#173c32]/15 bg-white px-3 py-2 text-xs font-semibold">Use demo photograph</button></label><label className="rounded-2xl border border-dashed border-[#173c32]/25 bg-[#f7f3ec] p-5 text-sm font-semibold"><span className="flex items-center gap-2"><Upload className="h-4 w-4" /> Drawing ({awkward ? "required" : "optional"})</span><input type="file" accept="image/*,.pdf" onChange={(event) => update("drawingName", event.target.files?.[0]?.name ?? "")} className="mt-3 block w-full text-xs font-normal" /></label><p className="sm:col-span-2 text-xs leading-5 text-[#6f7f79]">This staging proof records file names only. Secure upload storage is an outstanding Shopify/API dependency.</p></div></div>}
