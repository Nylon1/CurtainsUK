...
function DirectionCard({ direction, fabrics, final, onReaction, onOutcome, commerceUrl }: { direction: Direction; fabrics: Record<string, Fabric | null | 'error'>; final: boolean; onReaction: (reaction: string) => void; onOutcome: (card: Card, direction: Direction, event: string) => Promise<boolean>; commerceUrl: (card: Card, direction: Direction, sample: boolean) => string }) {
  ...
  if (direction.cards.length > 1) return <article className={`${styles.directionCard} ${styles.styleDirectionCard}`}><div className={styles.styleDirectionIntro}><p className={styles.eyebrow}>{direction.label}</p><h2>{direction.purpose}</h2></div><div className={styles.styleFabricGrid}>{direction.cards.map((entry) => <StyleFabricCard key={entry.fabricMasterId} card={entry} direction={direction} fabric={fabrics[entry.fabricMasterId]} onOutcome={onOutcome} commerceUrl={commerceUrl} />)}</div></article>;
...
function StyleFabricCard({ card, direction, fabric, onOutcome, commerceUrl }: { card: Card; direction: Direction; fabric?: Fabric | null | 'error'; onOutcome: (card: Card, direction: Direction, event: string) => Promise<boolean>; commerceUrl: (card: Card, direction: Direction, sample: boolean) => string }) {
