...
import { STOREFRONT_FABRICS, compositionLabel, repeatLabel } from "@/lib/storefront/fabrics";
...
export default function FabricBrowser({ windowSlug }: { windowSlug?: string }) {
  const [query, setQuery] = useState("");
  const [saved, setSaved] = useState<string[]>([]);
  const visible = useMemo(() => {
    const search = query.trim().toLowerCase();
    if (!search) return STOREFRONT_FABRICS;
    return STOREFRONT_FABRICS.filter((fabric) => [fabric.collection, fabric.design, fabric.colour, fabric.storefrontDescription].some((value) => value.toLowerCase().includes(search)));
  }, [query]);

  function handleSample(fabricId: string) {
    const fabric = STOREFRONT_FABRICS.find((item) => item.id === fabricId);
    if (!fabric) return;
    saveSample({ fabricId, sku: fabric.sample.sku, design: fabric.design, colour: fabric.colour, windowSlug });
          ...
          <article key={fabric.id} className="overflow-hidden rounded-[26px] border border-[#173c32]/10 bg-white shadow-[0_12px_40px_rgba(23,60,50,0.07)]">
            ...
            <div className="p-5">
              ...
              <div className="mt-5 grid gap-2">
                <button onClick={() => handleSample(fabric.id)} className="inline-flex min-h-11 items-center justify-center gap-2 rounded-full border border-[#173c32]/15 px-4 text-sm font-semibold hover:border-[#173c32]/35"><PackageOpen className="h-4 w-4" />{saved.includes(fabric.id) ? <><Check className="h-4 w-4" /> Saved</> : "Order Sample"}</button>
