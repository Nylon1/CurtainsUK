...
The private query preserves existing PT Standard-price selection and uses SDG Cut Price; validated GBP evidence, latest explicit approval and no future-dated observations are required. The governed PT PDF importer is the sole Cut-only PT source for its authorised cohort and takes precedence there; older Webtex or bulk Cut-only evidence remains outside Browse pricing. No value is relabelled or derived. Approved prices remain valid until superseded/revoked under existing policy; historical price_expires_at metadata is not a new expiry gate. Amounts are rounded to minor units before multiplication. No cost, source-price field, margin, snapshot or raw supplier evidence is sent to the browser.
