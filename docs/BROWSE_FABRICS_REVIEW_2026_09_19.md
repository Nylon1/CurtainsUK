...
The private query follows the verified-supplier-price policy: owner-confirmed PT Cut Price ex VAT / SDG Cut Price; validated GBP evidence; latest explicit approval; no future-dated observations. Existing PT Standard-only observations remain a temporary compatibility fallback until refreshed with Cut evidence; they are not relabelled or derived. Approved prices remain valid until superseded/revoked under existing policy; historical price_expires_at metadata is not a new expiry gate. Amounts are rounded to minor units before multiplication. No cost, source-price field, margin, snapshot or raw supplier evidence is sent to the browser.
