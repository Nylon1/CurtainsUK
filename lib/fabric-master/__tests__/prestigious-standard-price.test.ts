...
test('PT retains established Standard Price selection and accepts Cut-only evidence', () => {
...
test('a newer approved Standard Price preserves established PT selection over Cut-only history', () => {
  const snapshots = [
  ...
  assert.equal(selectCurrentApprovedSupplierCostMinor({ supplierId: 'prestigious-textiles', snapshots, promotionEvents, now: new Date('2026-09-17T00:00:00Z') }), 3100);
});
