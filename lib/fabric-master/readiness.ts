...
export function fabricReadiness(record: FabricMasterRecord, evidence: {
  stock?: PublicSupplierAvailability; stale?: boolean; priceConfirmed?: boolean; shippingKnown?: boolean;
} = {}) {
  ...
  const sampleReady = sampleEligible && currentStockConfirmed;
  ...
  return {policyVersion:FABRIC_READINESS_POLICY,canonical,browsable,recommendationEligible:browsable,
    ...
    stockMessage:discontinued ? 'No longer available' : currentStockConfirmed ? 'Fabric available' : evidence.stock === 'TEMPORARILY_UNAVAILABLE' && evidence.stale === false ? 'Out of stock — awaiting supplier stock' : 'Check availability',
  } as const;
}
