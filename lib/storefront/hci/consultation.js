(() => {
  ...
  const publishCustomerEvent = (name, data = {}) => {
    try {
      if (window.Shopify?.analytics?.publish) window.Shopify.analytics.publish(name, data);
    } catch {
      /* Measurement must never interrupt the consultation. */
    }
  };
  publishCustomerEvent('curtainsuk:fabric_intelligence_started', {
    entry: new URLSearchParams(location.search).get('entry') || 'guided',
  });
  const labels = {
    LOVE: "Love",
    LIKE: "Like",
  ...
  function offerImage() {
    stage.innerHTML = `<p class="eyebrow">Your inspiration · optional</p><h1>Have something you’d like us to match with?</h1><p>One image is enough. You can also skip and continue with questions.</p><div class="reference-panel"><label>Reference type<select id="reference-type">${["room", "paint", "sofa-upholstery", "wallpaper", "rug", "flooring", "existing-fabric", "moodboard"].map((t) => `<option>${t}</option>`).join("")}</select></label><label>Choose one image<input id="reference-file" type="file" accept="image/jpeg,image/png,image/webp"></label><p class="fine">JPG, PNG or WebP, up to 2 MB. Your image is used privately to help match fabrics to your room. It is discarded after analysis, sooner than our 30-day maximum. <a href="/curtainsuk-image-privacy" target="_blank" rel="noopener">How we use your image</a>.</p><button class="primary" id="upload">Find my colours</button></div><button class="quiet" id="skip">${view.phase === "complete" ? "Skip — show my recommendations" : "Skip — continue with questions"}</button>`;
  ...
  function palette() {
    ...
    stage.querySelector("#confirm").onclick = () => {
      publishCustomerEvent('curtainsuk:palette_created', {sessionId: view.sessionId});
      edit({ type: "confirm" });
    };
  }
    ...
    if (view.phase === "discovery") {
          ...
          .map(async (d) => {
            ...
            try {
              ...
              if (!f?.browseReady || f.id !== item.fabricMasterId)
                throw Error();
              return `<article class="card"><h2>${esc(d.label)}</h2><p>${esc(d.purpose)}</p><img loading="lazy" src="${esc(f.images[0].url)}?width=700" alt="${esc(f.design + " " + f.colour)}"><p class="eyebrow">${esc(f.brand)}</p><h3>${esc(f.design)} · ${esc(f.colour)}</h3><p><b>Why we chose this</b> ${esc(item.explanation.join(" "))}</p><p>${esc(f.availability || "Check availability")}</p>${!f.sampleAvailable ? '<p><a href="/pages/fabric-library">Explore fabric alternatives</a></p>' : ""}${final ? "" : reactions(d, item, "strategyReaction") + reactions(d, item, "fabricReaction")}<div class="actions">${f.sampleAvailable === true ? `<a class="secondary" href="${esc(link(f, item, d, true))}">Order Sample</a>` : "<p>Sample availability to confirm</p>"}<a class="primary" href="${esc(link(f, item, d, false))}">Make Curtains</a></div></article>`;
            } catch {
      ...
      stage.querySelectorAll('.actions a').forEach(a => {
        a.onclick = async event => {
          ...
          if (await act({type:'outcome',event:a.textContent === 'Order Sample' ? 'SAMPLE_INTENT' : 'FABRIC_SELECTED',fabricMasterId:context.fabricMasterId,strategyId:context.strategyId})) {
            if (a.textContent !== 'Order Sample') publishCustomerEvent('curtainsuk:fabric_selected', {
              sessionId: view.sessionId,
              fabricMasterId: context.fabricMasterId,
