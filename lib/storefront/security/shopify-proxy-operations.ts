export const SHOPIFY_PROXY_OPERATIONS = ["catalog", "price", "specialist-review", "review-request", "review-acceptance", "checkout-handoff", "sample-order", "consultation", "consultation-asset", "premium-asset", "premium-catalog", "premium-session", "premium-command", "hci-session", "hci-command", "image-privacy"] as const;
...
export const SHOPIFY_PROXY_OPERATION_POLICY: Record<ShopifyProxyOperation, {
  ...
  'premium-catalog': {methods:['GET'],maximumBytes:0,rateLimit:{limit:120,windowSeconds:60}},
  'premium-session': {methods:['POST'],maximumBytes:2048,rateLimit:{limit:20,windowSeconds:900}},
  'premium-command': {methods:['POST'],maximumBytes:3_000_000,rateLimit:{limit:120,windowSeconds:60}},
