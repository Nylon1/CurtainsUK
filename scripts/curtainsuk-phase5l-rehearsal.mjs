...
for (const [name,configuration] of [
  ...
  for (const [key,value] of Object.entries({configuration:JSON.stringify(configuration),calculation:JSON.stringify(calculation.data),contactName:`Phase 5L ${name}`,contactEmail:'phase5e-rehearsal@curtainsuk.invalid',contactPhone:'',notes:'Synthetic staging rehearsal. Do not fulfil or contact.'})) form.set(key,value);
  ...
  const transition = (toState) => api(`/api/admin/reviews/${requestId}/transition`,{toState,expectedState:current.reviewState,latestRevisionId:latest.revisionId,reason:`Phase 5L ${toState}: synthetic staff rehearsal`},staff.cookie);
  ...
  current=moved.data.review; latest=current.revisions.at(-1);
  const ready = await api(`/api/admin/reviews/${requestId}/checkout`,{expectedState:'APPROVED',revisionId:latest.revisionId,reason:'Phase 5L readiness gate rehearsal'},staff.cookie);
...
writeFileSync('artifacts/phase5l/email-evidence-rehearsal.json',JSON.stringify(report,null,2)+'\n');
