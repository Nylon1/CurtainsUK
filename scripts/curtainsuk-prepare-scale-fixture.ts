...
import { supplierFacts } from '../lib/fabric-master/browse-experience';
...
async function main() {
  ...
  for (const brand of ["sanderson", "morris-co", "harlequin", "zoffany", "scion", "clarke-clarke"]) {
    const result = await db.from("fabric_colourways").select("fabric_id").eq("brand_id", `sdg-${brand}`).neq("lifecycle_state", "DISCONTINUED").order("fabric_id").limit(125);
  ...
  for (let start = 0; start < ids.length; start += 24) {
    ...
    for (const r of records) {
      const safe = projectCustomerSafeFabric(r), p = profiles.data.find(v => v.fabric_id === r.fabric_id);
      // Expanded importer keys include image identity; select the exact MAIN by
      // canonical fabric ID instead of assuming the legacy one-image key.
      const m = (Object.values(media.mappings) as import("../lib/fabric-master/supplier-media").ImportedMedia[]).find(image => image.fabricId === r.fabric_id && image.imageType === "MAIN" && (!image.mediaScope || image.mediaScope === "COLOURWAY"));
