...
async function main() {
  ...
  for (let start = 0; start < ids.length; start += 24) {
    ...
    for (const r of records) {
      ...
      fabrics.push({ id: safe.id, supplier: safe.supplier, brand: safe.brand, collection: safe.collection, design: safe.design, colour: safe.colour, composition: safe.composition, usableWidthMm: safe.usableWidthMm, fullWidthMm: safe.fullWidthMm, verticalRepeatMm: safe.verticalRepeatMm, horizontalRepeatMm: safe.horizontalRepeatMm, patternMatchType: safe.patternMatchType, sampleAvailable: safe.sampleAvailable, availability: safe.availability, configurable: false, configurationMessage: "Price and availability to be confirmed", launchReady: false, feedEligible: false, description: p?.description_validated ? p.description : "Editorial and current supplier verification pending. Internal scale-test record only.", colourFamilies: p?.colour_families ?? ["UNKNOWN"], patterns: p?.patterns ?? ["UNKNOWN"], characters: p?.characters ?? ["UNKNOWN"], styles: p?.styles ?? ["UNKNOWN"], images, imageReferences: images.map(i => i.url), metadata: retailMetadata(r), headings: [], windowTypes: [], linings: [], rooms: [], careInstructions: [], weightGsm: r.weight_gsm });
    }
  }
