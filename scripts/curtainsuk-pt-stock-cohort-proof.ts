...
async function main() {
  const rows = JSON.parse(process.env.PT_PROOF_COHORT_JSON ?? "[]") as { sku: string; collection: string }[];
  if (rows.length !== 50 || new Set(rows.map(r => r.sku)).size !== 50 || rows.some(r => !/^\d{4}\/\d{3}$/.test(r.sku) || !r.collection.trim())) throw new Error("PT_COHORT_PROOF_INVALID");
