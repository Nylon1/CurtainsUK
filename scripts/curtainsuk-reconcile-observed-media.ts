...
async function main(){
 ...
 if(process.argv.includes("--apply-colour-corrections")) {
  ...
  const path=process.argv.find(a=>a.startsWith("--colour-corrections="))?.slice("--colour-corrections=".length) ?? "artifacts/portal-discovery/colour-corrections.json";
  const corrections=JSON.parse(await readFile(path,"utf8")) as {resolution:string;rows:{master_id:string;workbook_sku:string;portal_sku:string;brand:string;design:string;portal_design?:string;workbook_colour:string;portal_colour:string;expected_revision:string;workbook_effective_date:string}[]};
  const age=Date.now()-Date.parse(observed.checkedAt);
  if(!corrections.rows.length || corrections.rows.length>250 || corrections.rows.some(c=>c.brand!==observed.brand) || new Set(corrections.rows.map(c=>c.master_id)).size!==corrections.rows.length || !Number.isFinite(age) || age<0 || age>86400000) throw new Error("CORRECTION_SCOPE_INVALID");
  for(const correction of corrections.rows) {
   if(correction.workbook_sku!==correction.portal_sku || !observed.rows.some(r=>r[0]===correction.portal_sku && r[1]===`${correction.portal_design ?? correction.design} ${correction.portal_colour}`)) throw new Error("EXACT_PORTAL_EVIDENCE_REQUIRED");
 ...
 for(const [sku,title,imagePath] of observed.rows){
  ...
  const knownDisplayAlias=clarkeDisplayAlias || scionDisplayAlias;
  const identityMatches=normal(m.brand_name)===normal(observed.brand) && (formattedTitle || knownDisplayAlias);
  reconciliation.push({master_id:m.fabric_id,workbook_sku:m.supplier_sku,portal_sku:sku,master_design:m.design_name,master_colour:m.colour_name,portal_title:title,portal_status:observed.status,match_method:identityMatches?(exactTitle?"EXACT_SKU_BRAND_DESIGN_COLOUR":formattedTitle?"EXACT_SKU_BRAND_DISPLAY_FORMATTING":"EXACT_SKU_BRAND_COLOUR_VERIFIED_DISPLAY_ALIAS"):"UNRESOLVED_IDENTITY_DIFFERENCE",confidence:identityMatches?"HIGH":"WITHHELD",resolution:identityMatches?"APPROVED_MEDIA_ONLY":"WITHHELD",verified_at:observed.checkedAt});
  ...
  const later=discovery.observations.some(o=>o.route===route && Date.parse(o.checkedAt)>Date.parse(observed.checkedAt));
  if(!later) discovery=recordDiscoveryObservation(discovery,{route,state:"IN_PROGRESS",location,checkedAt:observed.checkedAt,pagesVisited:1,paginationExhausted:false,productFound:true,exactSkuFound:true,colourwayMedia:1,designMedia:0,ambiguous:false});
