...
const scaleFixture = process.argv.includes("--scale-1000");
const port = scaleFixture ? 3217 : 3216;
let fixtureCache: Awaited<ReturnType<typeof catalogue>> | undefined;
async function catalogue() {
  if (scaleFixture) {
    const fixture = JSON.parse(await readFile("artifacts/phase5h/private/scale-fixture-1000.json", "utf8")) as Awaited<ReturnType<typeof searchRetailFabrics>>["fabrics"];
    if (fixture.length !== 1000 || fixture.some(f => f.configurable || f.launchReady)) throw new Error("SCALE_FIXTURE_UNSAFE");
    return fixture;
  }
  ...
  return [...products.values()].map((product) => {
    ...
    return { ...safe, supplierSku: undefined, imageReferences: images.map((i) => i.url), images, description: factualRetailDescription(record), metadata: retailMetadata(record), headings: [], windowTypes: [], careInstructions: [], colourFamilies: ["UNKNOWN"], patterns: ["UNKNOWN"], characters: ["UNKNOWN"], styles: ["UNKNOWN"], launchReady: false, configurable: false };
...
createServer(async (request, response) => {
  ...
  if (request.method !== "GET" || ![`127.0.0.1:${port}`, `localhost:${port}`].includes(request.headers.host ?? "")) { response.writeHead(403).end(); return; }
  try {
    ...
    if (url.pathname === "/api/catalog") {
      const all = scaleFixture ? (fixtureCache ??= await catalogue()) : await catalogue();
      ...
      const filtered = all.filter((f) => (!params.get("query") || `${f.design} ${f.colour} ${f.collection}`.toLowerCase().includes(params.get("query")!.toLowerCase()))
        ...
        && (!params.get("colour") || f.colourFamilies.includes(params.get("colour")!))
        && (!params.get("pattern") || f.patterns.includes(params.get("pattern")!))
        && (!params.get("style") || f.styles.includes(params.get("style")!))
        && (!params.get("character") || f.characters.includes(params.get("character")!))
      ...
      const result = params.has("fabric") ? { fabric: all.find((f) => f.id === params.get("fabric")) ?? null } : { fabrics: filtered.slice((page - 1) * 24, page * 24), page, pageSize: 24, total: filtered.length, pages: Math.ceil(filtered.length / 24), facets: { brands: [...new Set(all.map(f => f.brand))].sort(), collections: [...new Set(all.map((f) => f.collection))].sort(), ...RETAIL_TAXONOMY } };
