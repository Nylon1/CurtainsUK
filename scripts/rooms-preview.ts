...
import { retainCurtain, reviewHouse, type HouseReviewRequest } from '../lib/storefront/rooms-core';
...
import { prepareHouseCheckout, houseCheckoutCustomerResult, type HouseCheckoutServices } from '../lib/storefront/rooms-checkout';
import { roomsCustomerError } from '../lib/storefront/rooms-core';
import mediaEvidence from './rooms-preview-media.json';
...
let scenario='normal';
const services:HouseCheckoutServices={
  secret,now:()=>new Date().toISOString(),
  async productionFabric(id){const f=sources.find(f=>f.id===id);if(!f)throw Error('NOT_FOUND');return{supplierSku:f.supplierReference,identity:{supplier:f.supplier,brand:'Prestigious Textiles',design:f.design,colour:f.colour}};},
  ...
  async stock(_id,metres){return scenario!=='out-of-stock'&&(scenario!=='combined-stock'||metres<15);},
...
const server=createServer(async(req,res)=>{try{
  ...
  if(url.pathname==='/preview-evaluation'){
    const fabricId=url.searchParams.get('fabric')||sources[0].id;
    const configuration:StagingPriceRequest={fabricId,windowSlug:'standard-window',hardware:'TRACK',measurementBasis:'TRACK_WIDTH',heading:'WAVE',widthCm:180,dropCm:220,lining:'BLACKOUT',construction:'PAIR',stackDirection:'SPLIT',desiredFinish:'FLOOR'};
    const calculation=await services.calculate(configuration);res.setHeader('Content-Type','application/json');return res.end(JSON.stringify({configuration,calculation}));
  }
  ...
  if(url.pathname==='/preview-api/rooms'&&req.method==='POST'){
    const buffers:Buffer[]=[];let length=0;for await(const chunk of req){length+=chunk.length;if(length>524288)throw Error('BODY_TOO_LARGE');buffers.push(chunk);}const input=JSON.parse(Buffer.concat(buffers).toString());res.setHeader('Content-Type','application/json');
    if(input.action==='review')return res.end(JSON.stringify(await reviewHouse(input.payload as HouseReviewRequest,services)));
    if(input.action==='retain')return res.end(JSON.stringify(await retainCurtain(input.payload,services)));
    if(input.action==='checkout')return res.end(JSON.stringify(houseCheckoutCustomerResult(await prepareHouseCheckout(input.payload,services))));
    throw Error('PREVIEW_ACTION_DENIED');
  }
  res.setHeader('Content-Type','text/html; charset=utf-8');
  if(url.pathname==='/'||url.pathname==='/pages/build-my-rooms')return res.end(html(rooms));
  if(url.pathname==='/pages/curtain-visualiser')return res.end(html(`<main class="cukrooms" data-local-make data-rooms-enabled="true"><p class="cukrooms__eyebrow">Local Make Curtains handoff fixture</p><h1>Make Curtains</h1><p>This fixture exercises the existing Add to my rooms integration with a server-priced example: 180 × 220 cm, fitted track, Wave, Blackout, Pair. It does not reproduce the live configurator.</p><form data-cuk-checkout-form><h2>Add your curtain</h2><button class="cukrooms__primary" type="submit">Add to my rooms</button></form><p role="alert" data-make-error></p></main><script>window.addEventListener('load',()=>{const root=document.querySelector('[data-local-make]');CurtainsUKRoomsFlow.prepare(root);root.querySelector('form').onsubmit=async(event)=>{event.preventDefault();const button=event.target.querySelector('button');button.disabled=true;try{const fabric=new URLSearchParams(location.search).get('fabric')||'pt-4269-147';const evaluation=await(await fetch('/preview-evaluation?fabric='+encodeURIComponent(fabric))).json();await CurtainsUKRoomsFlow.add(evaluation,'/preview-api',{roomId:event.target.elements.roomId.value,roomName:event.target.elements.roomName.value});}catch(error){root.querySelector('[data-make-error]').textContent=error.message;button.disabled=false;}}});</script>`));
...
}catch(error){res.writeHead(400,{'Content-Type':'application/json'});res.end(JSON.stringify({error:roomsCustomerError(error)}));}});
