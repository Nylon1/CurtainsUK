...
reset();
report.walkthroughBlocked='Video encoding unavailable in this runtime. Updated screenshot walkthrough supplied.';
...
assert.deepEqual(evaluate('window.__roomsRenderStates'),['CONTENT']);assert.match(evaluate('document.querySelector(".cukrooms__intro").innerText'),/Your home is taking shape[\s\S]*1 room · 1 window/);assert.match(evaluate('document.querySelector(".cukrooms__saved").innerText'),/Take your time/);report.checks.push('Saved arrival renders populated on its first DOM update with no empty-state flash');
const exactFabric=one.rooms[0].curtains[0].fabric_master_id;assert.equal(evaluate('document.querySelector(".cukrooms__fabric-detail").getAttribute("href")'),'/pages/fabric-library?view=browse-fabrics&fabric='+encodeURIComponent(exactFabric));assert.match(evaluate('document.querySelector(".cukrooms__specs").textContent'),/Size/);assert.equal(evaluate('document.querySelector("[data-cuk-rooms]").innerText.includes("—")'),false);report.checks.push('Exact governed fabric-details route, Size label and authored copy punctuation');
...
// Screenshot evidence is the review artifact; no video encoder dependency.
...
assert.deepEqual(evaluate('[...document.querySelectorAll("[data-room-choice]")].map(b=>b.dataset.roomChoice)'),['Bedroom','Dining Room','Kitchen','Home Office','Child’s Room','Other Room']);
run(['find','role','button','click','--name','Kitchen','--exact']);run(['snapshot','-i']);assert.equal(current().rooms.at(-1).room_name,'Kitchen');assert.equal(evaluate('CurtainsUKRooms.readIntent().room_id'),current().rooms.at(-1).room_id);run(['find','role','button','click','--name','Back to my rooms']);run(['reload']);assert.equal(current().rooms.at(-1).room_name,'Kitchen');assert.deepEqual(evaluate('window.__roomsRenderStates'),['CONTENT']);report.checks.push('Six next-room choices with labelled photographic inspiration; selected room and intent persist before discovery, including refresh');
