...
const exactFabric=one.rooms[0].curtains[0].fabric_master_id;assert.equal(evaluate('document.querySelector(".cukrooms__fabric-detail").getAttribute("href")'),'/pages/fabric-library?view=browse-fabrics&fabric='+encodeURIComponent(exactFabric));assert.match(evaluate('document.querySelector(".cukrooms__specs").innerText'),/Size/);assert.equal(evaluate('document.querySelector("[data-cuk-rooms]").innerText.includes("—")'),false);report.checks.push('Exact governed fabric-details route, Size label and authored copy punctuation');
