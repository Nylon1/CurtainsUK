(function () {
  ...
  document.querySelectorAll('[data-cuk-rooms]').forEach(root=>{
    ...
    function curtainVisual(curtain) {
      ...
      return `<div class="cukrooms__visual">${headingAsset?`<img class="cukrooms__heading-main" width="1024" height="1024" src="${escape(headingAsset)}" alt="Neutral ${escape(human(curtain.configuration.heading))} heading study, not your chosen fabric or room" loading="lazy">`:'<div class="cukrooms__visual-empty">Heading study unavailable.</div>'}<div class="cukrooms__visual-label"><strong>${escape(human(curtain.configuration.heading))} heading study</strong><span>Not your chosen fabric or room.</span></div><a class="cukrooms__fabric-inset" href="${BROWSE}&fabric=${encodeURIComponent(curtain.fabric_master_id)}">${url?`<img src="${escape(url)}" alt="${escape(curtain.fabric.design)}, ${escape(curtain.fabric.colour)} exact fabric photograph" width="300" height="300" loading="lazy">`:'<span>Fabric photograph unavailable</span>'}<span>Your chosen fabric ↗</span></a></div>`;
    }
    function card(curtain, room) {
      return `<article class="cukrooms__card" data-curtain="${escape(curtain.configuration_id)}">${curtainVisual(curtain)}<div class="cukrooms__detail"><p class="cukrooms__eyebrow">${escape(curtain.window_name)}</p><h3>${escape(curtain.fabric.design)}, ${escape(curtain.fabric.colour)}</h3><p class="cukrooms__colour">${escape(curtain.fabric.brand || curtain.fabric.supplier)}</p><div class="cukrooms__price"><strong>${money(curtain.last_validated_price)}</strong><small>VAT included</small></div><dl class="cukrooms__specs">${spec(curtain).map(([key,value])=>`<div><dt>${key}</dt><dd>${escape(value)}</dd></div>`).join('')}</dl><div class="cukrooms__card-actions"><button class="cukrooms__remove" data-remove="${escape(curtain.configuration_id)}" data-room="${escape(room.room_id)}" aria-label="Remove curtain from ${escape(room.room_name)}, ${escape(curtain.window_name)}">Remove curtain</button><a class="cukrooms__fabric-detail" href="${BROWSE}&fabric=${encodeURIComponent(curtain.fabric_master_id)}">View fabric details →</a></div><p class="cukrooms__card-saved"><span aria-hidden="true">✓</span> Saved on this device. Price and availability are checked again at review.</p><a class="cukrooms__reuse" href="/pages/curtain-visualiser?fabric=${encodeURIComponent(curtain.fabric_master_id)}&rooms_new=1" data-reuse>Use this fabric in another room ↗</a></div></article>`;
    }
    function reviewCard(line) {
      const retained=house.rooms.flatMap(room=>room.curtains).find(curtain=>curtain.configuration_id===line.configuration_id);
      const fabric=line.fabric || retained?.fabric;
      const configuration=line.configuration || retained?.configuration;
      return `<article class="cukrooms__review-line" data-review-curtain="${escape(line.configuration_id)}">${retained?curtainVisual(retained):''}<div class="cukrooms__review-detail"><p class="cukrooms__eyebrow">${escape(line.room_name)}, ${escape(line.window_name)}</p>${fabric?`<h3>${escape(fabric.design)}, ${escape(fabric.colour)}</h3>`:''}<p class="cukrooms__review-price">${line.currentPrice!=null?money(line.currentPrice):'Price needs review'}<small>${line.status==='BLOCKED'?'Needs attention':line.currentPrice!=null?'VAT included':''}</small></p>${!line.configuration&&retained?'<p class="cukrooms__notice">Saved details shown. These could not be verified.</p>':''}${configuration?`<dl class="cukrooms__specs">${spec({configuration}).map(([key,value])=>`<div><dt>${escape(key)}</dt><dd>${escape(value)}</dd></div>`).join('')}</dl>`:''}<p class="cukrooms__line-status">${line.status==='PRICE_CHANGED'?`The price has changed from ${money(line.previousPrice)} to ${money(line.currentPrice)}.`:escape(line.message)}</p>${line.status==='PRICE_CHANGED'?`<label class="cukrooms__check"><input type="checkbox" data-price-ack="${escape(line.configuration_id)}">I accept this updated price.</label>`:''}</div></article>`;
    }
    function renderReview() {
      if(!review) return '';
      return `<section class="cukrooms__review" id="rooms-review" tabindex="-1"><p class="cukrooms__eyebrow">One last look</p><h2>Review My Rooms</h2><p>${review.ready?'Your rooms are ready to review.':'Some curtains need your attention. All your rooms remain saved.'}</p><div class="cukrooms__review-cards">${review.lines.map(reviewCard).join('')}</div><div class="cukrooms__review-totals"><dl><div><dt>Curtains subtotal</dt><dd>${review.goods!=null?money(review.goods):'Awaiting review'}</dd></div><div><dt>Delivery</dt><dd>${review.delivery!=null?money(review.delivery):'Awaiting review'}</dd></div><div class="cukrooms__total"><dt>House total</dt><dd>${review.total!=null?money(review.total):'Awaiting review'}</dd></div></dl><p>${review.vat!=null?`Includes ${money(review.vat)} VAT.`:'VAT included.'}</p></div><p>${escape(review.deliveryMessage)}</p><label class="cukrooms__check"><input type="checkbox" data-confirm-measurements>I confirm the measurements and selections shown above are correct.</label><p class="cukrooms__notice">Made to your specification. Please check each curtain carefully. To change a curtain, remove it and configure it again.</p><p class="cukrooms__notice">After ordering, need to request a change? Email us within 2 hours at <a href="mailto:enquiries@curtainsuk.com">enquiries@curtainsuk.com</a>. We’ll review your request and get in touch. Changes are not guaranteed; statutory rights remain unaffected.</p><button class="cukrooms__primary" data-checkout disabled>Continue to secure checkout</button>${!review.checkoutEnabled?'<p class="cukrooms__notice">Preview only. Checkout is not open yet.</p>':''}</section>`;
    }
