-- PT base price is approved Standard Price ex VAT. Other suppliers retain their existing basis.
-- Replace readiness consumers only; raw evidence and approved/order snapshots are unchanged.
...
begin
  ...
  where fabric.fabric_id = p_snapshot->>'fabric_master_id'
    ...
    and (case when supplier_snapshot.supplier_id = 'prestigious-textiles' then supplier_price.standard_trade_price else supplier_price.cut_trade_price end) > 0
...
begin
  if not exists (
    ...
    where snapshot.snapshot_id = p_snapshot_id
      ...
      and (case when snapshot.supplier_id = 'prestigious-textiles' then price.standard_trade_price else price.cut_trade_price end) > 0
