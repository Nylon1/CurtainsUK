CREATE OR REPLACE FUNCTION curtainsuk_private.search_retail_fabrics(p_filters jsonb, p_page integer, p_size integer, p_guide_min integer, p_guide_max integer)
 ...
 SET search_path TO ''
AS $function$
  with knowledge as materialized (select fabric_id,visual_fields from curtainsuk_private.fabric_visual_knowledge where knowledge_state in ('COMPLETE','PARTIAL_GOVERNED')), approved_guides as materialized (
