CREATE MATERIALIZED VIEW curtainsuk_private.fabric_knowledge_discovery_cache AS
 SELECT jsonb_build_object('knowledgeOptions',r->'knowledgeOptions','brands',r->'brands','collections',r->'collections') data,now() refreshed_at
 FROM (SELECT curtainsuk_private.search_retail_fabrics('{"knowledge":"1"}'::jsonb,1,1,null,null) r) q;
 REVOKE ALL ON curtainsuk_private.fabric_knowledge_discovery_cache FROM PUBLIC,anon,authenticated;
 GRANT SELECT ON curtainsuk_private.fabric_knowledge_discovery_cache TO service_role;
...
AS $function$
  with knowledge as materialized (select fabric_id,visual_fields from curtainsuk_private.fabric_visual_knowledge_read_cache where knowledge_state in ('COMPLETE','PARTIAL_GOVERNED')), latest_promotions as materialized (
 ...
 select s.* from curtainsuk_private.supplier_snapshots s join latest_promotions e using(snapshot_id)
 where s.validation_status='VALIDATED' and s.checked_at<=now() and e.promotion_state='APPROVED_FOR_PROJECTION'
    ...
    'knowledgeOptions',case when p_filters->>'knowledge'='1' then (select data->'knowledgeOptions' from curtainsuk_private.fabric_knowledge_discovery_cache) else '[]'::jsonb end,
    'brands',(select data->'brands' from curtainsuk_private.fabric_knowledge_discovery_cache),
    'collections',(select data->'collections' from curtainsuk_private.fabric_knowledge_discovery_cache));
$function$
;
