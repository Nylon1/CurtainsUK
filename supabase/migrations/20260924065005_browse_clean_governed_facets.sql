-- Clean governed Browse facet projection. It uses the current Fabric Master,
-- current_retail_guide_prices(), existing stock/media rules, and private knowledge cache.
-- Unknown knowledge remains absent: it is never inferred or treated as a match.
...
WITH filter_values AS (
  ...
  SELECT c.fabric_id,c.supplier_id,c.supplier_sku,g.guide_minor,c.colour_name,
    ...
    CASE WHEN jsonb_typeof(vk.visual_fields->'secondaryColours'->'value')='array' THEN
      coalesce((SELECT jsonb_agg(lower(x.value)) FROM jsonb_array_elements_text(vk.visual_fields->'secondaryColours'->'value') x(value)),'[]'::jsonb) ELSE '[]'::jsonb END AS secondary_colours,
...
COMMENT ON FUNCTION curtainsuk_private.search_retail_fabrics(jsonb,integer,integer,integer,integer) IS 'Browse governed facets from current Fabric Master, guide-price helper, approved media, stock projection, and private Fabric Knowledge cache.';
