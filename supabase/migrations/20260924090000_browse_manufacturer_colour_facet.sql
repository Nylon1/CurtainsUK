-- Colour is a supplier/manufacturer fact. This helper maps the preserved exact
-- manufacturer colourway into the existing Browse colour-family vocabulary without
-- mutating Fabric Master. Ambiguous names remain unmapped rather than guessed.
CREATE OR REPLACE FUNCTION curtainsuk_private.manufacturer_colour_families(p_colourway text)
RETURNS text[]
LANGUAGE sql
IMMUTABLE
SET search_path TO ''
AS $function$
  SELECT array_remove(ARRAY[
    CASE WHEN lower(coalesce(p_colourway,'')) ~ '(white|snow|porcelain|cream|ivory|vanilla|linen|chalk|oyster|parchment|pearl|cloud|ecru|calico|canvas|feather|shell|polar|pampas|angora)' THEN 'white/cream' END,
    CASE WHEN lower(coalesce(p_colourway,'')) ~ '(beige|stone|sand|biscuit|fawn|taupe|truffle|mushroom|putty|oatmeal|flax|hessian|almond|latte|mink|pumice|biscotti|jute)' THEN 'beige/taupe' END,
    CASE WHEN lower(coalesce(p_colourway,'')) ~ '(grey|gray|silver|slate|charcoal|graphite|smoke|pebble|dove|pewter|mist|sterling|steel|flint|mercury|ash|chrome|platinum|quartz|granite|shale|shadow|storm)' THEN 'grey' END,
    CASE WHEN lower(coalesce(p_colourway,'')) ~ '(black|ebony|onyx|noir|jet)' THEN 'black' END,
    CASE WHEN lower(coalesce(p_colourway,'')) ~ '(blue|navy|indigo|denim|aqua|turquoise|cobalt|midnight|peacock|sky|kingfisher|lagoon|duckegg|duck egg|ink|chambray|marine|azure|seafoam|ocean|atlantic|sapphire|woad|cornflower|pacific)' THEN 'blue' END,
    CASE WHEN lower(coalesce(p_colourway,'')) ~ '(green|sage|olive|moss|fern|emerald|teal|forest|willow|jade|chartreuse|peppermint|fennel|apple|lime|laurel|basil|ivy|palm|avocado|matcha|pine|eau de nil|lichen)' THEN 'green' END,
    CASE WHEN lower(coalesce(p_colourway,'')) ~ '(pink|rose|blush|coral|heather|petal|blossom|fuchsia|woodrose)' THEN 'pink' END,
    CASE WHEN lower(coalesce(p_colourway,'')) ~ '(red|crimson|scarlet|ruby|claret|burgundy|wine|berry|russet|auburn|brick|rouge|madder|poppy|sienna)' THEN 'red' END,
    CASE WHEN lower(coalesce(p_colourway,'')) ~ '(purple|violet|plum|lilac|lavender|aubergine|damson|amethyst|grape|orchid|thistle|fig)' THEN 'purple' END,
    CASE WHEN lower(coalesce(p_colourway,'')) ~ '(orange|terracotta|rust|paprika|ochre|spice|copper|ginger|cinnamon)' THEN 'orange' END,
    CASE WHEN lower(coalesce(p_colourway,'')) ~ '(yellow|gold|mustard|saffron|daffodil|honey|citron|citrus|topaz|amber|butterscotch|harvest)' THEN 'yellow/gold' END,
    CASE WHEN lower(coalesce(p_colourway,'')) ~ '(brown|mocha|chocolate|cocoa|chestnut|tobacco|camel|espresso|umber|bark|earth|oak|nutmeg)' THEN 'brown' END,
    CASE WHEN lower(coalesce(p_colourway,'')) ~ '(natural|mineral|antique)' THEN 'neutral' END,
    CASE WHEN lower(coalesce(p_colourway,'')) ~ 'multi' THEN 'multicolour' END
  ], NULL)::text[];
$function$;

REVOKE ALL ON FUNCTION curtainsuk_private.manufacturer_colour_families(text) FROM public, anon, authenticated;
GRANT EXECUTE ON FUNCTION curtainsuk_private.manufacturer_colour_families(text) TO service_role;
...
WITH filter_values AS (
  ...
  SELECT c.fabric_id,c.supplier_id,c.supplier_sku,g.guide_minor,c.colour_name,
    ...
    curtainsuk_private.manufacturer_colour_families(c.colour_name) AS manufacturer_colour_families,
    lower(coalesce(vk.visual_fields->'primaryColour'->>'value','')) AS visual_primary_colour,
    CASE WHEN jsonb_typeof(vk.visual_fields->'secondaryColours'->'value')='array' THEN
      coalesce((SELECT jsonb_agg(lower(x.value)) FROM jsonb_array_elements_text(vk.visual_fields->'secondaryColours'->'value') x(value)),'[]'::jsonb) ELSE '[]'::jsonb END AS visual_secondary_colours,
...
COMMENT ON FUNCTION curtainsuk_private.search_retail_fabrics(jsonb,integer,integer,integer,integer) IS 'Browse query using Fabric Master colourway and current guide-price manufacturer facts; Fabric Knowledge supplies Pattern, Texture, Character and Finish facets only.';
