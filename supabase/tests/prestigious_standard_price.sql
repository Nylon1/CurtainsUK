...
begin
 foreach n in array array['fabric_commercial_evidence','create_staging_configuration_snapshot','promote_fabric_for_staging_projection','current_retail_guide_prices'] loop
  ...
  if definition not like '%pt-pdf-cut:%' or definition not like '%cut_trade_price%' or definition not like '%standard_trade_price%' then raise exception 'PT governed-PDF Cut / Standard compatibility gate absent: %',n; end if;
 end loop;
...
if definition not like '%pt-pdf-cut:%' or definition not like '%cut_trade_price%' or definition not like '%standard_trade_price%' then raise exception 'Browse PT governed-PDF Cut / Standard source is invalid'; end if;
