...
begin
 foreach n in array array['fabric_commercial_evidence','create_staging_configuration_snapshot','promote_fabric_for_staging_projection','current_retail_guide_prices'] loop
  ...
  if definition not like '%cut_trade_price%' or definition not like '%standard_trade_price%' then raise exception 'PT Cut preference / Standard fallback gate absent: %',n; end if;
 end loop;
 ...
 if definition not like '%cut_trade_price%' or definition not like '%standard_trade_price%' then raise exception 'Browse PT Cut preference / Standard fallback source is invalid'; end if;
